...
:: Change directory to the batch file's directory (root)
cd /d "%~dp0"
...
if exist Server\data\cloudflared.log del /f /q Server\data\cloudflared.log

:: Start FastAPI server in a new window
echo Starting FastAPI server on port 499...
start "FastAPI Server" cmd /c "cd Server && venv\Scripts\python.exe app.py"
